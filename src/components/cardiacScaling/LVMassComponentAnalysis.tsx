...
import FormulaSelector, { useFormulaSelection, FormulaValuesDisplay } from '@/components/common/FormulaSelector';
import { 
 ...
 */
export const calculateLVMass = (ivs: number, lvd: number, lvpw: number): number => {
...
const generateComponentAnalysisData = (
  ...
  // Use EXACT same population range as other tabs
  ...
  const ivsAnalysis = generateScalingAnalysis(ivsData, formulaSelection, undefined, populationOptions);
  const lvdAnalysis = generateScalingAnalysis(lvdData, formulaSelection, undefined, populationOptions);
  const lvpwAnalysis = generateScalingAnalysis(lvpwData, formulaSelection, undefined, populationOptions);
  const massAnalysis = generateScalingAnalysis(massData, formulaSelection, undefined, populationOptions);
  ...
  const maxPopulationBSA = Math.max(...result.filter(p => p.bsa > 0).map(p => p.bsa));
  ...
  if (linearIsRatiometric || massIsRatiometric) {
    ...
    for (let bsa = Math.ceil(maxPopulationBSA * 10) / 10; bsa <= extendTo; bsa += 0.1) {
      result.push({
        bsa,
        ...
        maleCalculatedMass: linearIsRatiometric ? calculateLVMass(
          linearCoeffs.maleIvs * bsa,
          linearCoeffs.maleLvd * bsa,
          linearCoeffs.maleLvpw * bsa
        ) : NaN,
        femaleCalculatedMass: linearIsRatiometric ? calculateLVMass(
...
const LVMassComponentAnalysis: React.FC = () => {
  ...
  const analysisData = useMemo(() => {
    return generateComponentAnalysisData(linearConfigId, massConfigId, formulaSelection);
    ...
    <div>
      ...
      <header style={{ marginBottom: '2rem' }}>
        <hgroup>
          ...
          <p>
            Comparison of LV mass calculated from scaled linear components versus 
            direct mass scaling across different methodological approaches.
          </p>
        </hgroup>
      </header>
      ...
      <section style={{ marginBottom: '2rem' }}>
        <div style={{ 
          ...
          <div>
            ...
            <div style={{ fontSize: '0.875rem', color: 'var(--pico-muted-color)', marginTop: '0.25rem' }}>
        ...
        <div style={{
          ...
          <div className="chart-container">
            <header style={{ marginBottom: '1rem' }}>
              <h3 style={{ margin: '0 0 0.5rem 0' }}>Linear Component Measurements</h3>
              <p style={{ fontSize: '0.9rem', color: 'var(--pico-muted-color)', margin: 0 }}>
                IVS, LVD, LVPW scaled using {linearConfigs.find(c => c.id === linearConfigId)?.name}
              </p>
            </header>
            ...
            <ResponsiveContainer width="100%" height={400}>
              <LineChart data={analysisData}>
                ...
                <XAxis 
                  ...
                  label={{ value: 'Body Surface Area (m²)', position: 'insideBottom', offset: -5 }}
                />
                <YAxis 
                  domain={[0, 'dataMax']}
                  label={{ value: 'Measurement (cm)', angle: -90, position: 'insideLeft' }}
                />
                <Tooltip 
                  ...
                  labelFormatter={(value) => `BSA: ${typeof value === 'number' ? value.toFixed(1) : value} m²`}
                />
                
                {/* LVD Lines (Solid) */}
                {toggleState.lvd && (
                  <>
                    <Line dataKey="maleLvd" stroke={COLORS.male} strokeWidth={3} 
                          {...LINE_STYLES.lvd} dot={false} name="LVD Male" connectNulls={false} />
                    <Line dataKey="femaleLvd" stroke={COLORS.female} strokeWidth={3} 
                          {...LINE_STYLES.lvd} dot={false} name="LVD Female" connectNulls={false} />
                  </>
                )}
                ...
                {toggleState.ivs && (
                  <>
                    <Line dataKey="maleIvs" stroke={COLORS.ivs.male} strokeWidth={2} 
                          {...LINE_STYLES.ivs} dot={false} name="IVS Male" connectNulls={false} />
                    <Line dataKey="femaleIvs" stroke={COLORS.ivs.female} strokeWidth={2} 
                          {...LINE_STYLES.ivs} dot={false} name="IVS Female" connectNulls={false} />
                  </>
                )}
                ...
                {toggleState.lvpw && (
                  <>
                    <Line dataKey="maleLvpw" stroke={COLORS.lvpw.male} strokeWidth={2} 
                          {...LINE_STYLES.lvpw} dot={false} name="LVPW Male" connectNulls={false} />
                    <Line dataKey="femaleLvpw" stroke={COLORS.lvpw.female} strokeWidth={2} 
                          {...LINE_STYLES.lvpw} dot={false} name="LVPW Female" connectNulls={false} />
                  </>
                )}
            ...
            <div style={{ fontSize: '0.875rem', color: 'var(--pico-muted-color)', marginTop: '0.5rem' }}>
              <strong>Line styles:</strong> LVD (solid), IVS (dashed), LVPW (dotted)
            </div>
          </div>
          ...
          <div className="chart-container">
            ...
            <ResponsiveContainer width="100%" height={400}>
              <LineChart data={analysisData}>
                ...
                <YAxis 
                  domain={[0, 'dataMax']}
                  label={{ value: 'LV Mass (g)', angle: -90, position: 'insideLeft' }}
                />
                <Tooltip 
                  formatter={(value, name) => [
                ...
                <Line dataKey="malePublishedMass" stroke={COLORS.male} strokeWidth={3} 
                      {...LINE_STYLES.published} dot={false} name="Published Male" connectNulls={false} />
                <Line dataKey="femalePublishedMass" stroke={COLORS.female} strokeWidth={3} 
                      {...LINE_STYLES.published} dot={false} name="Published Female" connectNulls={false} />
                
                {/* Calculated Mass Lines (Dashed) */}
                <Line dataKey="maleCalculatedMass" stroke={COLORS.male} strokeWidth={3} 
                      {...LINE_STYLES.calculated} dot={false} name="Calculated Male" connectNulls={false} />
                <Line dataKey="femaleCalculatedMass" stroke={COLORS.female} strokeWidth={3} 
                      {...LINE_STYLES.calculated} dot={false} name="Calculated Female" connectNulls={false} />
            ...
            <div style={{ fontSize: '0.875rem', color: 'var(--pico-muted-color)', marginTop: '0.5rem' }}>
              <strong>Line styles:</strong> Published (solid), Calculated from components (dashed)
            </div>
          </div>
        </div>
      </section>
      ...
      <section style={{
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '1.5rem' }}>
          ...
          <div>
            <h4>ASE Formula</h4>
            <p style={{ fontSize: '0.9rem', fontFamily: 'var(--pico-font-family-monospace)' }}>
